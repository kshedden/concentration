...
bandwidth, the covariate dimension, and the first and second
derivatives of the true mean function being estimated (ref).  It is
likely that these same factors influence the bias of QNN, but in
addition the bias of QNN is also influenced by the probability point
$p$, with bias anticipated to increase as $p$ moves away from 0.5.
...
\equiv Q_y(p) - Q_y(0.5)$.  We conducted a simulation study in which
Gaussian data from a heteroscedastic population were simulated as
follows.  The explanatory variables $x$ were simulated as standard
independent multivariate Gaussian values of dimension $d$.  The
conditional mean of the simulated population was either $x_1$ or
$x_1^2$, and the conditional variance was $\sigma^2(1 + x_2^2)$.
...
$0.9$ for each simulated dataset.  The estimation errors were then used 
as the dependent variables in a least squares regression, with the 
following mean structure
...
generally took the form of attenuation toward
the conditional median -- that is, when $g_p(x) > 0$ the bias was
negative and with $g_p(x) < 0$ the bias was positive.  To summarize
the bias, we considered a relative bias based on the OLS fits, defined
as $|{\rm bias}(p, x) / g_p(x)|$.  For simplicity we focus on results
for $p=0.75$ and take the median of the fitted relative bias values
over the $n$ observations.  These values were found to increase with
$\sigma$, with $d$, and when the true quantile functions have
curvature.  However in the situations that we considered, the relative bias
tended to be less than 10\% when $d=2$ and less than 20\% when $d=5$.
=#
        ...
        for square in [false, true]
            for sigma in [1.0, 2]
                ...
                for p in [0.5, 0.75, 0.9]
                    xx = simstudy1(x, sigma, p, square; nrep = 5, la = 0.1)
                    push!(xa, xx)
                end

                # True conditional medians
                q50 = xa[1][:, :tquant]
